...
public class MAXSwerveModule {
  ...
  private final ProfiledPIDController m_turningPIDController = new ProfiledPIDController(ModuleConstants.kPModuleTurningController, 0, 0);
   ...
   * Encoder.
   */
  public MAXSwerveModule(int drivingCANId, int turningCANId, int encoderID, double chassisAngularOffset) {
    m_drivingSparkMax = new CANSparkMax(drivingCANId, MotorType.kBrushless);
    // ~ m_turningVictorSPX = new CANSparkMax(turningCANId, MotorType.kBrushless);
    //!m_turningVictorSPX = new WPI_VictorSPX(turningCANId);
    m_turningSparkMax = new CANSparkMax(turningCANId, MotorType.kBrushed);

    // Factory reset, so we get the SPARKS MAX to a known state before configuring
    // them. This is useful in case a SPARK MAX is swapped out.
    m_drivingSparkMax.restoreFactoryDefaults();
    //?m_turningVictorSPX.configFactoryDefault();
    m_turningSparkMax.restoreFactoryDefaults();

    // Setup encoders and PID controllers for the driving and turning SPARKS MAX.
    m_drivingEncoder = m_drivingSparkMax.getEncoder(); //? Debug from here down
    //! Fix the issue below by debugging this line
    ...
    m_turningVictorSPX.setNeutralMode(ModuleConstants.kTurningMotorIdleMode);
  ...
  public SwerveModuleState getState() {
    ...
    return new SwerveModuleState(m_drivingEncoder.getVelocity(),
        new Rotation2d(m_turningEncoder.getAbsolutePosition() - m_chassisAngularOffset));
  }
  ...
  public void setDesiredState(SwerveModuleState desiredState) {
    ...
    m_turningPIDController.setSetpoint(optimizedDesiredState.speedMetersPerSecond);
